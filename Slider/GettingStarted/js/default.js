    ...
    app.onactivated = function (args) {
        if (args.detail.kind === activation.ActivationKind.launch) {
            ...
            args.setPromise(WinJS.UI.processAll().then(function () {
            	...
            	//it is not related to the sliders functionality. 
            	var radios = document.querySelectorAll(".options input");
            	for (var i = 0; i < radios.length; i++) {
            		radios[i].addEventListener("click", function (e) {
            			var sliderId = e.currentTarget.value;
            			var sliderWrappers = document.getElementById("sliders").children;
            			for (var i = 0; i < sliderWrappers.length; i++) {
            				sliderWrappers[i].style.display = "none";
            			}
